                  ...
                  {[
                    ...
                    <div key={item.key} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                      ...
                      <Switch
                        ...
                        onCheckedChange={(checked: boolean) => setSettings({ ...settings, [item.key]: checked })}
                      />
                    </div>
                  ))}
                </div>
        ...
        >
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                { icon: Upload, label: '充值', color: '#00D6C2' },
                { icon: Download, label: '提现', color: '#18FF74' },
                { icon: TrendingUp, label: '账单', color: '#FFD700' },
                { icon: CreditCard, label: '银行卡', color: '#FF2566' }
              ].map((action, i) => (
                <motion.button
                  key={i}
                  whileHover={{ scale: 1.05, y: -2 }}
                  whileTap={{ scale: 0.95 }}
                  onClick={() => toast.success(`${action.label}功能开发中...`)}
